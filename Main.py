...
async def train_model(
    ...
    try:
        ...
        try:
            await run_optuna(study, train_df, test_df, n_trials)
...
def get_or_train_model_sync(symbol, train_df, models_dir, best_params=None):
    ...
    if os.path.exists(f"{model_path}.zip"):
        ...
        tensorboard_log = os.path.join("./ppo_tensorboard/", f"run_{run_id}")
        ...
        if best_params:
            ...
            model = PPO(
                ...
                tensorboard_log=tensorboard_log,  # Use the UUID-based path
                verbose=1,
            )
        else:
            model = PPO(
...
def objective_sync(trial, train_df, test_df):
    try:
        logging.debug(f"Starting optimisation trial {trial.number}")
        ...
        tensorboard_log = os.path.join("./ppo_tensorboard/", f"trial_{trial_id}")
...
async def run_optuna(study, train_df, test_df, n_trials):
    ...
    for _ in range(n_trials):
        ...
        score = await loop.run_in_executor(
            executor, objective_sync, trial, train_df, test_df
        )
